...
export const TimerManager = ({ time, timerRunning }) => {
  ...
  return (
    <Row
      className={`${styles.timerContainer} align-items-center justify-content-center`}
    >
      <Col>
        <Button
          color={timerRunning ? 'primary' : 'success'}
          block
          onClick={toggleTimer}
        >
          <Localized
            name={
              timerRunning ? 'interface.pause_timer' : 'interface.start_timer'
            }
          />
        </Button>
      </Col>
